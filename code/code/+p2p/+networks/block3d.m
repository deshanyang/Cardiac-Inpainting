function layers = block3d(id, nChannels, direction, varargin)
...
%     parser.addParameter('KernelSize', 4);
    parser.addParameter('KernelSize', 3);
%     parser.addParameter('KernelSize', 5);
    parser.parse(nChannels, direction, varargin{:});
    inputs = parser.Results;
    
    switch inputs.direction 
        case 'down'
            conv = @convolution3dLayer;
            padName = 'Padding';
        case 'up'
            conv = @transposedConv3dLayer;
            padName = 'Cropping';
        otherwise
