function translatedImage = p2ptranslate(p2pModel, inputImage, varargin)
...
%     translatedImage = (extractdata(out) + 1)/2 * options.ARange;
        translatedImage = extractdata(out) * options.ARange;
...
%     networkInput = 2*single(inputImage)/options.ARange - 1;
    networkInput = single(inputImage)/options.ARange;
    ...
    if size(inputImage,3)>3
        networkInput = dlarray(networkInput, 'SSSCB');
    else
